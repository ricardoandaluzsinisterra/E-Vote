...
def get_poll_by_id(cursor, poll_id: str) -> Optional[Dict[str, Any]]:
    """
    Retrieve a poll by its ID with all associated options using a single JOIN query.

    Optimized to use a single database query by joining polls and options,
    then grouping results in Python for better performance.
    ...
    """
    try:
        # Single query with LEFT JOIN to fetch poll and its options
        query = """
        ...
        # Build poll data from first row
        poll_data = {
            "id": str(rows[0][0]),
            "title": rows[0][1],
            "description": rows[0][2],
            "created_by": rows[0][3],
            "created_at": rows[0][4].isoformat() if rows[0][4] else None,
            "expires_at": rows[0][5].isoformat() if rows[0][5] else None,
            "is_active": rows[0][6],
            "options": []
        }

        # Add all options from the joined rows
        for row in rows:
...
def get_user_polls(cursor, user_id: int) -> List[Dict[str, Any]]:
    """
    Get all polls created by a specific user with their options using a single JOIN query.
